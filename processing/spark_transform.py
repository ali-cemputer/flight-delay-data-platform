"""
spark_transform.py - BTS Bronze -> Silver -> Bigquery

What this scripts does step by step:
    1. Parse CLI arguments (year, month) via click
    2. Build SparkSession configured for GCS and BigQuery connectors
    3. Read partitioned Parquet files from the GCS bronze layer
    4. Cast columns to correct types (Date, Int, Boolean)
    5. Fill null delay values for cancelled flights with 0
    6. Join three lookup tables to enrich the dataset
    7. Add derived columns: is_delayed, delay_category
    8. Rename all columns to snake_case for BigQuery compatibility
    9. Write cleaned data to GCS silver layer (partitioned Parquet)
    10. Load silver data into BigQuery with partition and cluster config
"""
...
def main(year, month):
    ...
    # 1) read / write paths  ────────────────────────────────────────────────────
    read_path   = f"gs://{GCS_BUCKET}/{BRONZE_PREFIX}/year={year}/month={month}/data.parquet" #raw path
    ...
    "Dest": "string",
    "DestCityName": "string",
    "CRSDepTime": "int",       # hhmm -> int
    "DepTime": "int",          # hhmm -> int | actual departure time
    ...
    "ArrTime": "int",
    "ArrDelay": "int",
    "Cancelled": "int",        # float -> int, will be cast to bool after null fill
    "CancellationCode": "string",
    "Diverted": "int",         # 1=Yes, 0=No, will be cast to bool
    ...
    for col_name, dtype in cast_list.items():
        df = df.withColumn(col_name, F.col(col_name).cast(dtype))

    # Cast Cancelled and Diverted to bool after int conversion
    ...
    rename_map = {
        "Year": "year", "Month": "month", "DayOfWeek": "day_of_week",
        "FlightDate": "flight_date", "Reporting_Airline": "reporting_airline",
        "Origin": "origin", "OriginCityName": "origin_city_name",
        "Dest": "dest", "DestCityName": "dest_city_name",
        ...
        "Diverted": "diverted", "ActualElapsedTime": "actual_elapsed_time",
